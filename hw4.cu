...
__device__ void sha256_transform_kernel(SHA256 *ctx, const BYTE *msg)
{
...
__device__ void sha256_kernel(SHA256 *ctx, const BYTE *msg, size_t len)
{
	...
	{
		sha256_transform_kernel(ctx, &msg[i]);
	}
...
__device__ unsigned char decode_kernel(unsigned char c)
{
    switch(c)
    {
        case 'a':
...
__device__ void convert_string_to_little_endian_bytes_kernel(unsigned char* out, char *in, size_t string_len)
{
    ...
    {
        out[b] = (unsigned char)(decode_kernel(in[s])<<4) + decode_kernel(in[s+1]);
    }
}

// print out binary array (from highest value) in the hex format
__device__ void print_hex_kernel(unsigned char* hex, size_t len)
{
    for(int i=0;i<len;++i)
    {
        printf("%02x", hex[i]);
...
__device__ void print_hex_inverse_kernel(unsigned char* hex, size_t len)
{
...
__device__ int little_endian_bit_comparison_kernel(const unsigned char *a, const unsigned char *b, size_t byte_len)
{
...
__device__ void double_sha256_kernel(SHA256 *sha256_ctx, unsigned char *bytes, size_t len)
{
    SHA256 tmp;
    sha256_kernel(&tmp, (BYTE*)bytes, len);
    sha256_kernel(sha256_ctx, (BYTE*)&tmp, sizeof(tmp));
}
...
__global__ void reset_found_kernel() {
    d_found = false;
}

__global__ void nonce_search_kernel(HashBlock block, unsigned char *target_hex, unsigned int start_nonce, unsigned int step, unsigned int *result)
{
...
void solve(FILE *fin, FILE *fout,int  taskid)
{
